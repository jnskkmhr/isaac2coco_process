...
class BasicDataConverter:
    def __init__(self, root_dirs, save_dir, modality, HW):
    ...
    def register_ws(self, root_dir:str):
...
#     def __init__(self, root_dirs, save_dir, modality, HW):
#         # modality = ["rgb", "depth", "object_detection_npy", "object_detection", "depth_npy", "depth", "semantic_segmentation", "instance_segmentation", "instance_segmentation_mapping"]
#         self.super().__init__(root_dirs, save_dir, modality, HW)
#         self.CLASS2ID = {
#             "rock_middle":1, 
#             "rock_large":2, 
#             "UNLABELLED":0,
...
# class string to integer id mapping
CLASS2ID = {
    "rock":1,
    # "gravel":2,
    "ground":0,
...
class COCO_SegDataset(BasicDataConverter):
    ...
    def process_annotation(self, filename):
        ...
        for pixel_str in sub_masks.keys():
            ...
            sub_mask_pad = np.pad(sub_mask, 1, 'constant', constant_values=0) #[H+2, W+2]
            contours = find_contours(sub_mask_pad, 0.5, positive_orientation='low')
            segmentations, area, bbox, iscrowd, skip = self.mask_to_polygon(contours)
            if not skip:
...
# # WIP : multi-class segmentation for fast data processing
